...
class AddScriptPubKeyPatch():
    ...
    def run_patch(self):
        ...
        return 
        txs = self.session.query(Transaction.txid).filter(Transaction.has_multisig == None).all()
        count = 0
        for tx in txs:
            try:
                print "txid:", tx.txid, tx.__dict__
                tx_info = self.session.query(TransactionInfo). \
                    filter(TransactionInfo.txid == tx.txid).first()
                # Initialize scriptPubKeys
                tx.has_multisig = False
                tx.has_nulldata = False
                tx.has_pubkey = False
                tx.has_pubkeyhash = False
                tx.has_scripthash = False
            
                if tx_info.json_string:
                    tx_info_dict = json.loads(tx_info.json_string)
                    vouts = tx_info_dict[ATTRIBUTE_VOUT]
                    for vout in vouts:
                        script_type = vout[ATTRIBUTE_SCRIPT_PUB_KEY][ATTRIBUTE_TYPE]
                        print "script_type:", script_type
                        if script_type == ATTRIBUTE_MULTISIG:
                            tx.has_multisig = True
                        elif script_type == ATTRIBUTE_NULLDATA:
                            tx.has_nulldata = True
                        elif script_type == ATTRIBUTE_PUBKEY:
                            tx.has_pubkey = True
                        elif script_type == ATTRIBUTE_PUBKEYHASH:
                            tx.has_pubkeyhash = True
                        elif script_type == ATTRIBUTE_SCRIPTHASH:
                            tx.has_scripthash = True
                # Update transaction
